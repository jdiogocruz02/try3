...
Game::Game(const string& filename)
{
	...
	while (!file.eof())
	{
		for (int linen = 0; linen < maze.getnumRows(filename); linen++)
		{
			cout<< endl;
			for (int charn = 0; charn < maze.getnumCols(filename); charn++)
			{
				getline(file, line);
				this->mazeo[linen].push_back(line[charn]);
				if (line[charn] == 'R')
				{
